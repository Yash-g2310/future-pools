...
if [ $? -ne 0 ]; then
    ...
    if [[ -f "broadcast/DeployProofOfHumanWithBlacklist.s.sol/$CHAIN_ID/run-latest.json" ]]; then
...
BROADCAST_DIR="broadcast/DeployProofOfHumanWithBlacklist.s.sol/$CHAIN_ID"
...
if [ -n "$CELOSCAN_API_KEY" ]; then
    ...
    forge verify-contract --constructor-args $CONSTRUCTOR_ARGS --chain-id $CHAIN_NAME $CONTRACT_ADDRESS src/ProofOfHumanWithBlacklist.sol:ProofOfHumanWithBlacklist --watch
